...
let urlList = [
  "http://127.0.0.1:3000/template1",
  "http://127.0.0.1:3000/template2",
  "http://127.0.0.1:3000/template3",
];
let errorNameList = [
  "代理异常_Response Code",
  "代理异常_Response Timeout",
  "JS异常_IMAGE_LOAD_ERROR",
  "加载页面异常_ERR_ABORTED",
];
...
function getNewUser() {
  ...
  for (
    let hour = moment().startOf("day").unix();
    hour <= moment().endOf("day").unix();
    hour += 3600
  ) {
    sql += `
    (${id++}, 1, ${count[id % count.length]}, '${moment
      .unix(hour)
      .format("YYYY-MM-DD_HH")}', 'hour', ${id}, ${hour}, ${hour}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
}
function getDuration() {
  ...
  for (
    ...
    sql += `
    (${id++}, 1, ${stay[id % stay.length]}, ${id % 5}, '${moment
      .unix(hour)
      .format("YYYY-MM-DD_HH")}', 'hour', ${id}, ${hour}, ${hour}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
...
function getBrowserDistribution() {
  ...
  for (let browser of browserList) {
    for (let version of versions) {
      sql += `
      (${id++}, 1, '${browser}', '${version}', ${
        ((id * 2) % 10) + 5
      }, '${month}', ${id}, ${moment().unix()}, ${moment().unix()}),`;
    }
  }
  return sql.slice(0, sql.length - 1) + ";";
}
...
function getIndicatorSQl() {
  ...
  for (
    let timeAt = nowMoment.clone().subtract(3, "hour").unix();
    timeAt <= nowMoment.unix();
    timeAt += 60
  ) {
    sql += `
    (${id++}, ${id % 10}, ${id % 5}, '${
      indicatorList[id % indicatorList.length]
    }', '${urlList[id % urlList.length]}', ${id}, '${moment
      .unix(timeAt)
      .format("YYYY-MM-DD_HH:mm")}', 'minute', ${timeAt}, ${timeAt}),`;
  }
  for (
    let timeAt = nowMoment.clone().subtract(48, "hour").unix();
    timeAt <= nowMoment.unix();
    timeAt += 3600
  ) {
    sql += `
    (${id++}, ${id % 10}, ${id % 5}, '${
      ...
      .unix(timeAt)
      .format("YYYY-MM-DD_HH")}', 'hour', ${timeAt}, ${timeAt}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
}
function getOriginMonitorSQL() {
  ...
  for (
    let logAt = startDayMoment.unix(), count = 1;
    logAt <= nowMoment.unix() && count <= 100;
    logAt += 120, count++
  ) {
    sql += `
    (${id++}, '8', '${
      errorNameList[id % errorNameList.length]
    }', '400', ${id}, '10', '10', '10', '${
      urlList[id % urlList.length]
    }', '中国', '河南', '安阳', '${logAt}', 'md5', ${logAt}, ${logAt}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
}
function getBehaviorDistribution() {
  ...
  for (let day = threeDayAgo; day <= nowDay; day++) {
    for (let hour = 0; hour <= 23; hour++) {
      sql += `
      (${id++},1, '${code[id % code.length]}', '${name[id % name.length]}', '${
        urlList[id % urlList.length]
      }', ${id % 30}, '${
        nowMoment.format("YYYY-MM-") +
        day.toString().padStart(2, "0") +
        "_" +
        hour.toString().padStart(2, "0")
      }', 'day', ${id}, ${moment().unix()}, ${moment().unix()}),
      `;
    }
    sql += `
    (${id++},1, '${code[id % code.length]}', '${name[id % name.length]}', '${
      ...
      nowMoment.format("YYYY-MM-") + day.toString().padStart(2, "0")
    }', 'day', ${id}, ${moment().unix()}, ${moment().unix()}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
}
function getErrorSummarySQL() {
  ...
  for (let hour = endHour; hour <= nowHour; hour++) {
    ...
    for (let minute = 0; minute < 60; minute++) {
      ...
      sql += `(${id++}, '8', '${errorNameList[id % errorNameList.length]}', '${
        urlList[id % urlList.length]
      }', ${id}, '${
        timeStrPrefix + "_" + hourStr + ":" + minuteStr
      }', 'minute', ${id % 10}, ${moment().unix()}, ${moment().unix()}),
      `;
    }
    sql += `(${id++}, '8', '${errorNameList[id % errorNameList.length]}', '${
      urlList[id % urlList.length]
    }', ${id}, '${timeStrPrefix + "_" + hourStr}', 'hour', ${
      id % 100
    }, ${moment().unix()}, ${moment().unix()}),`;
  }
  for (let day = nowMoment.date(), count = 1; count <= 30; count++) {
    sql += `
    (${id++}, '8', '${errorNameList[id % errorNameList.length]}', '${
      urlList[id % urlList.length]
    }', ${id}, '${
      monthPrefix + "-" + day.toString().padStart(2, "0")
    }', 'day', ${id % 100}, ${moment().unix()}, ${moment().unix()}),`;
  }
  return sql.slice(0, sql.length - 1) + ";";
}
let TABLE_TEMPLATE = {};
...
TABLE_TEMPLATE[MUILT_T_O_MONITOR] = `(
  ...
  \`error_name\` varchar(64) NOT NULL DEFAULT '' COMMENT '错误名/错误代码,用于细分错误类别',
  ...
  \`url\` varchar(64) NOT NULL DEFAULT '' COMMENT '发生异常的url',
...
TABLE_TEMPLATE[MUILT_T_R_PERFORMANCE] = `(
  ...
  \`url\` varchar(64) NOT NULL DEFAULT '' COMMENT '页面url',
  \`city_distribute_id\` bigint(20) NOT NULL DEFAULT '0' COMMENT '城市分布详情记录id',
...
TABLE_TEMPLATE[MUILT_T_R_ERROR_SUMMARY] = `(
  ...
  \`error_name\` varchar(64) NOT NULL DEFAULT '' COMMENT '错误名字',
  \`url_path\` varchar(64) NOT NULL DEFAULT '' COMMENT '错误URL_PATH',
...
TABLE_TEMPLATE[SINGLE_T_O_ALARM_CONFIG] = `(
  ...
  \`error_name\` varchar(64) NOT NULL DEFAULT '' COMMENT '要报警错误名字',
  ...
  \`note\` varchar(64) NOT NULL DEFAULT '' COMMENT '配置说明',
...
class GenerateSQL extends Base {
  ...
  async execute(args, options) {
    ...
    let finishAtMoment = moment(
      finishAtYm,
      DATE_FORMAT.COMMAND_ARGUMENT_BY_MONTH
    );
    let sqlContent = "-- --------分表部分---------";
